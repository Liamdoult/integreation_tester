...
``` python
from intergration_tester import mongo_driver
...
class MongoDBDriver(driver.Driver):
    ...
    1. Initialise a new instance of MongoDB with Docker on initialisation of
    this object.
    2. Ready check that checks if the MongoDB service inside the container is
    running and not just the container itself.
    3. A reset that completely resets the MongoDB service to factory settings.
    ...
    def ready(self) -> bool:
        """ Confirm if the MongDB Service is running.
        ...
        method. This method servce no information purpose other than starting
